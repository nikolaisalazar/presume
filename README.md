...
- Browser persistence for resume data and formatting constraints, with visible failure/retry states.
- Session history for text, structure, and formatting, including undoable restore/reset.
...
**Download backup** creates `presume-backup.json` with resume text and formatting settings. **Restore backup** validates the file before asking to replace the current document. Older bare resume JSON files remain accepted; their confirmation explains that current formatting settings will be kept. Invalid or unsupported backups leave the document unchanged. Backups exclude Review, history, theme, zoom, and measured scale. Browser data remains local to this browser/profile. Undo/Redo retain up to 100 action groups while the app stays open, including internal navigation; history ends on reload. Reset replaces both text and formatting with defaults and can be undone.

Browser saving is transitional: it still writes two LocalStorage keys and cannot prevent other tabs from overwriting newer work. An untouched sample is not saved. Failed writes keep your draft in memory and show Retry saving; unreadable saved data is preserved until you explicitly choose to replace it. Use Download backup to keep an independent copy.
