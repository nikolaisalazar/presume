...
| T2: Document state and editing | 19 | Plain-text editing, accessible field semantics, reliable persistence and save status, tab conflict handling, structural undo, complete backup/restore. Owns F03, F04, F05, F07, F09; supplies undo for F01 | T2-1 complete backups integrated; T2-2 session/history implemented with handoff; T2-3 editing adapter is next |
...
and continue at T2-3 after integrating [T2-2](../remediation/T2_2_SESSION_HANDOFF.md).
Later UI packages use the session, revision, focus/command, and prepared-snapshot contracts published by T2-2; T2-3 must settle the full input adapter before the document component handoff to T3.
