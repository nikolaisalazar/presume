...
Two concrete contrast defects were corrected. Light accent text on the accent surface measured 4.400:1; reusing the approved `--primary-hover` token raises it to 5.552:1. A fresh context-isolated review then found custom Verdigris-only focus outlines measuring 1.747–1.960:1 against adjacent Light surfaces. Pairing those outlines with the existing `--focus-contrast` application edge or theme-independent `--paper-ink` document edge restores the required contrast while preserving the approved Verdigris cue. The existing Border Notch transition remains because it is purposeful, limited to 220ms, and disabled under reduced motion; a transform rewrite would change the approved interaction without evidence of a performance or accessibility defect.

An earlier independent review of Phase E checkpoint `b0346c76f0bddb7c9667d146c848676d1781793b` found no remaining issues after its shadow-token remediation. A later context-isolated review of PR head `8cf79feb52b1653ca3da496f8e18803112c90b53` found the focus-edge and test-boundary defects now remediated. Final exact-head re-review and merge evidence remain pending for PR #40, and the program is not complete before that closeout.
